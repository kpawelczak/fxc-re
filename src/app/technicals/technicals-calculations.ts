...
export class TechnicalsCalculationsComponent implements OnChanges {
	...
	@Output()
	calculatorPivotResults: EventEmitter<Array<number>> = new EventEmitter();

	@Output()
	calculatorFiboResults: EventEmitter<Array<number>> = new EventEmitter();

	private pivotResults: Array<number> = [];
	private fiboResults: Array<number> = [];

	private high: number;
	private low: number;
	private range: number;
	private result_236: number;
	private result_382: number;
	private result_50: number;
	private result_618: number;
